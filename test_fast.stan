...
transformed data {
  ...
  vector[N_all] all_counts;
  vector[N_all] all_bkg;
  vector[N_all] all_bkg_err;

  int all_non_zero[N_all];
  int all_zero[N_all];
  ...
  for (n in 1:N_intervals) {
    ...
    for (m in 1:N_dets[n]) {
      ...
      //      print(num_elements(all_counts[iter : iter + N_channels_used[n,m] -1 ]));
      //  print(num_elements(selected_counts));

      
      all_counts[iter : iter + N_channels_used[n,m] -1 ] = selected_counts;
      all_bkg[iter : iter + N_channels_used[n,m] -1] = selected_background;
      all_bkg_err[iter:iter + N_channels_used[n,m] -1] = selected_background_errors;
      
      
      for (i in 1:N_bkg_zero[n,m]) {
	
	all_zero[iter2 + i ]  = idx_background_zero[n,m, : N_bkg_zero[n,m]][i] + iter2;
	
      }
      
      
      for (i in 1:N_bkg_nonzero[n,m]) {
	
	all_non_zero[iter3 + i]  = idx_background_nonzero[n,m, : N_bkg_nonzero[n,m]][i] + iter3;
      ...
      iter+=N_channels_used[n,m];
      iter2+=N_bkg_zero[n,m];
      iter3+=N_bkg_nonzero[n,m];
...
model {
  ...
  for (n in 1:N_intervals) {
    ...
    for (m in 1:N_dets[n]) {
      ...
      all_expectation[iter4 : iter4 + N_channels_used[n,m] -1] = selected_expectation';
      
    }
    
  }

  target += pgstat(all_counts, all_bkg, all_bkg_err, all_expectation, all_zero[:iter2-1], all_non_zero[:iter3-1]  );
