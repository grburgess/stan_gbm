...
max_n_chans = np.max( [ [ len(d.observed_counts)  for d in  ds] for ds in all_data_sets])
max_n_echans = np.max( [ [ d.response.matrix.shape[1]  for d in  ds] for ds in all_data_sets])
...
pystan.stan_rdump(data_dict,'test_data.R')
